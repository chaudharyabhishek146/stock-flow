...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const product = await db.execute({
    ...
    args: [productId, session.orgId],
  });
  if (product.rows.length === 0) return Response.json({ error: 'Product not found.' }, { status: 404 });

  const skuConflict = await db.execute({
    sql: 'SELECT id FROM products WHERE organization_id = ? AND sku = ? AND id != ?',
    args: [session.orgId, normalizedSku, productId],
  });
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const result = await db.execute({
    sql: 'DELETE FROM products WHERE id = ? AND organization_id = ?',
    args: [parseInt(id, 10), session.orgId],
  });

  if (result.rowsAffected === 0) return Response.json({ error: 'Product not found.' }, { status: 404 });
  return Response.json({ success: true });
}
